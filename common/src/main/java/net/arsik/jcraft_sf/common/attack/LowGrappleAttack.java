...
public class LowGrappleAttack<A extends IAttacker<? extends A, ?>> extends AbstractSimpleAttack<LowGrappleAttack<A>, A> {
    public LowGrappleAttack(int cooldown, int windup, int duration, float moveDistance, float damage, int stun,
                               float hitboxSize, float knockback, float offset) {
        super(cooldown, windup, duration, moveDistance, damage, stun, hitboxSize, knockback, offset);
    }

    @Override
    public @NonNull MoveType<LowGrappleAttack<A>> getMoveType() {
        return LowGrappleAttack.Type.INSTANCE.cast();
    }
    ...
    @Override
    protected @NonNull LowGrappleAttack<A> getThis() {
        return this;
    }

    @Override
    public @NonNull LowGrappleAttack<A> copy() {
        return copyExtras(new LowGrappleAttack<>(getCooldown(), getWindup(), getDuration(), getMoveDistance(),
                getDamage(), getStun(), getHitboxSize(), getKnockback(), getOffset()));
    }

    public static class Type extends AbstractSimpleAttack.Type<LowGrappleAttack<?>> {
        public static final LowGrappleAttack.Type INSTANCE = new LowGrappleAttack.Type();

        @Override
        protected @NonNull App<RecordCodecBuilder.Mu<LowGrappleAttack<?>>, LowGrappleAttack<?>> buildCodec(RecordCodecBuilder.Instance<LowGrappleAttack<?>> instance) {
            return attackDefault(instance, LowGrappleAttack::new);
        }
    }
}
